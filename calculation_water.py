...
Здесь N — количество действий (доливок воды),
и каждая операция требует константного времени
для выполнения основных шагов: подсчёт прошедшего времени,
расчёт остаточной воды и пополнение запасов.

Цель использования класса состоит в том, чтобы логически разделить задачу на две части:
Класс отвечает за внутреннюю логику устройства (увлажнитель).
Внешняя функция занимается чтением и обработкой вводимых данных.
Такая структура улучшает понимание кода и облегчает поддержку и расширение функциональности в будущем.
Хотя в данном случае класс не является необходимым (личная инициатива) он может быть полезен в более сложных задачах.
'''
